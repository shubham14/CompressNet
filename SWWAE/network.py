...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        out = self.pad1(x)
        out = self.cnn1(out)
        out1, ind1 = self.maxpool(out)
        out = self.cnn2(out1)
        out2, ind2 = self.maxpool(out)
        out = self.cnn3(out2)
        out3, ind3 = self.maxpool(out)
        out = self.cnn4(out3)
        out4, ind4 = self.maxpool(out)
        out = self.cnn5(out4)
        out = self.pad2(out)
        out = self.cnn6(out)
        return out, [out1, out2, out3, out4], [ind1, ind2, ind3, ind4]
...
class Decoder_Orig(nn.Module):
    ...
    def __init__(self, in_channels, filters=[960, 480, 240, 120, 60]):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels, filters[0], 3),
                                   nn.ReLU(),
                                   nn.InstanceNorm2d(filters[0]))
        self.conv_tr1 = nn.Sequential(nn.ConvTranspose2d(filters[0], filters[1], 3, stride=2, padding=1, output_padding=1),
                                      nn.ReLU(),
                                      nn.InstanceNorm2d(filters[1]))
        self.conv_tr2 = nn.Sequential(nn.ConvTranspose2d(filters[1], filters[2], 3, stride=2, padding=1, output_padding=1),
                                      nn.ReLU(),
                                      nn.InstanceNorm2d(filters[2]))        
        self.conv_tr3 = nn.Sequential(nn.ConvTranspose2d(filters[2], filters[3], 3, stride=2, padding=1, output_padding=1),
                                      nn.ReLU(),
                                      nn.InstanceNorm2d(filters[3]))
        self.conv_tr4 = nn.Sequential(nn.ConvTranspose2d(filters[3], filters[4], 3, stride=2, padding=1, output_padding=1),
                                      nn.ReLU(),
                                      nn.InstanceNorm2d(filters[4]))
        self.tanh = nn.Tanh()
...
class Decoder_upsampled(nn.Module):
    '''
    ...
    '''
    def __init__(self, in_channels, filters=[960, 480, 240, 120, 60]):
        super(Decoder_upsampled, self).__init__()
        res_block = ResidualNet(filters[0], filters[0], kernel_size=3)
        self.res = nn.Sequential(res_block,
                                nn.ReLU())
        self.res_iter = nn.ModuleList([self.res for _ in range(9)])
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels, filters[0], 3),
                                   nn.ReLU(),
                                   nn.InstanceNorm2d(filters[0]))
        self.conv_tr1 = nn.Sequential(nn.Upsample(scale_factor=2, mode="bilinear"),
                                      nn.Conv2d(filters[0], filters[1], 3, stride=1, padding=1),
                                      nn.ReLU(),
                                      nn.InstanceNorm2d(filters[1]))
        self.conv_tr2 = nn.Sequential(nn.Upsample(scale_factor=2, mode="bilinear"),
                                      nn.Conv2d(filters[1], filters[2], 3, stride=1, padding=1),
                                      nn.ReLU(),
                                      nn.InstanceNorm2d(filters[2]))        
        self.conv_tr3 = nn.Sequential(nn.Upsample(scale_factor=2, mode="bilinear"),
                                      nn.Conv2d(filters[2], filters[3], 3, stride=1, padding=1),
                                      nn.ReLU(),
                                      nn.InstanceNorm2d(filters[3]))
        self.conv_tr4 = nn.Sequential(nn.Upsample(scale_factor=2, mode="bilinear"),
                                      nn.Conv2d(filters[3], filters[4], 3, stride=1, padding=1),
                                      nn.ReLU(),
                                      nn.InstanceNorm2d(filters[4]))
        self.tanh = nn.Tanh()
        self.pad1 = nn.modules.padding.ReflectionPad2d(1)      
...
class Decoder(nn.Module):
    ...
    def _initialize_weights_ICNR(self):
        kernel = self.ICNR(self.conv_tr1.weight, upscale_factor=2, inizializer=nn.init.kaiming_normal)
        self.conv_tr1.weight.data.copy_(kernel)
        kernel = self.ICNR(self.conv_tr2.weight, upscale_factor=2, inizializer=nn.init.kaiming_normal)
        self.conv_tr2.weight.data.copy_(kernel)
        kernel = self.ICNR(self.conv_tr3.weight, upscale_factor=2, inizializer=nn.init.kaiming_normal)
        self.conv_tr3.weight.data.copy_(kernel)
    ...
    def forward(self, x, ind):  
        ...
        out = self.MaxUnpool1(out4, ind[3])
        out = self.BatchNorm1(out)
        out = self.relu(out)

        # Second stage decoder conv block
        out3 = self.conv_tr2(out)
        out = self.MaxUnpool2(out3, ind[2])
        out = self.BatchNorm2(out)
        out = self.relu(out)

        # Third stage decoder conv block
        out2 = self.conv_tr3(out)
        out = self.MaxUnpool3(out2, ind[1])
        out = self.BatchNorm3(out)
        out = self.relu(out)

        # Fourth stage decoder conv block
        out1 = self.conv_tr4(out)
        out = self.MaxUnpool4(out1, ind[0])
...
class Decoder_new(nn.Module):
    ...
    def forward(self, x, ind):  
        ...
        # Second stage decoder conv block
        out3 = self.conv_tr2(out)
        out = F.max_unpool2d(out3, ind[2], 2)
        out = self.BatchNorm2(out)
        out = self.relu(out)

        # Third stage decoder conv block
        out2 = self.conv_tr3(out)
        out2 = F.max_unpool2d(out2, ind[1], 2)
