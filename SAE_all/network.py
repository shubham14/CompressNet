...
class Decoder(nn.Module):
	...
	def __init__(self, in_channels, filters=[960, 480, 240, 120, 60]):
		...
		self.conv_tr3 = nn.Sequential(nn.PixelShuffle(2),
									  ...
									  nn.ReLU())
		self.conv_tr4 = nn.Sequential(nn.PixelShuffle(2),
									  nn.Conv2d(filters[3]//4, filters[4], 3, stride=1, padding=1),
	...
	def _initialize_weights(self):
		init.orthogonal_(self.conv_tr1[1].weight, init.calculate_gain('relu'))
		init.orthogonal_(self.conv_tr2[1].weight, init.calculate_gain('relu'))
		init.orthogonal_(self.conv_tr3[1].weight, init.calculate_gain('relu'))
		init.orthogonal_(self.conv_tr4[1].weight)
	...
	def _initialize_weights_ICNR(self):
		...
		kernel = self.ICNR(self.conv_tr4[1].weight, upscale_factor=2, inizializer=nn.init.kaiming_normal)
		self.conv_tr4[1].weight.data.copy_(kernel)
